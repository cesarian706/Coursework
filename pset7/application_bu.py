...
def index():
    ...
    for row in rows:
        ...
        table_dict = {"symbol": symbol[0]['symbol'], "name": symbol[0]['name'], "shares": row['sum(shares)'], "price": "${:,.2f}".format(stock['price']), "total": "${:,.2f}".format(total)}
        #append to dict
    ...
    return render_template("portfolio.html", table=table, cash="${:,.2f}".format(cash), total_all="${:,.2f}".format(total_all))
...
def buy():
    ...
    if request.method == "POST":
        ...
        # show history with updated info
        rows_new = db.execute("SELECT * FROM portfolio JOIN stocks ON portfolio.stock_id = stocks.stock_id WHERE id = :id", id=session["user_id"])
        return render_template("history.html", rows = rows_new)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if str.isnumeric() == False:
            return apology("enter numeric value")
        # cast string to int, check that it is a positive value greater than 0
        if int(request.form.get("shares")) < 0:
            return apology("enter valid number of shares")
        ...
        for row in rows:
            if stock['symbol'] == symbol.upper():
                if row['sum(shares)'] >= int(request.form.get("shares")) and row['sum(shares)'] > 0:
                    ...
                    price = info['price']
                    price_total = info['price'] * int(request.form.get("shares"))
                    cash_new = "{:,.2f}".format(cash + price_total)
        ...
        # pass updated data to history html
        rows_new = db.execute("SELECT * FROM portfolio JOIN stocks ON portfolio.stock_id = stocks.stock_id WHERE id = :id", id=session["user_id"])
        return render_template("history.html", rows = rows_new)
    # display sell page if method GET
    else:
        return render_template("sell.html",table=table, cash="${:,.2f}".format(cash), total_all="${:,.2f}".format(total_all))